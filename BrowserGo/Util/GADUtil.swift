...
extension GADUtil {
    ...
    /// 展示
    func show(_ position: ADPosition, vc: UIViewController? = nil, completion: @escaping (ADBaseModel?)->Void) {
        // 超限需要清空广告
        if isADLimited {
            clean(.all)
        }
        ...
        case .interstitial:
            ...
            if let ad = loadAD?.loadedArray.first as? InterstitialADModel, !AppEnterBackground, !isADLimited {
                ad.impressionHandler = { [weak self] in
                    self?.add(.show)
                ...
                if !AppEnterBackground, !AppInActive {
                    ad.present(from: vc)
                } else {
                    completion(nil)
                }
